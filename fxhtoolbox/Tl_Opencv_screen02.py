...
class Tl_Opencv_screen02(object):
    ...
    # 使用scharr操作(指定ksize=-1)构造灰度图在水平和竖直方向上的梯度幅值表示
        gradX = cv2.Sobel(gray, ddepth=cv2.CV_32F, dx=1,
                          dy=0, ksize=-1)  # 对x方向求导
        gradY = cv2.Sobel(gray, ddepth=cv2.CV_32F, dx=0,
                          dy=1, ksize=-1)  # 对y方向求导
    ...
    # 然后进行二值化处理，要么是255(白)要么是0(黑)
        ...
        ret, thresh = cv2.threshold(
            blurred, 225, 255, cv2.THRESH_BINARY)  # 进行图像固定阈值二值化
